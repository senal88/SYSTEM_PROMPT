...
# ============================================
# Script de Coleta: System Prompt - Ubuntu
# ============================================
...
REPORT_FILE="$OUTPUT_DIR/system_prompt_ubuntu.json"
...
if [ -f "$GLOBAL_PROMPT" ]; then
    ...
    SIZE=$(stat -c%s "$GLOBAL_PROMPT" 2>/dev/null || echo "0")
...
if [ -f "$CURSOR_RULES" ]; then
    ...
    SIZE=$(stat -c%s "$CURSOR_RULES" 2>/dev/null || echo "0")
