...
# ============================================
# Script de Coleta: Configurações Cursor IDE - Ubuntu
# ============================================
...
REPORT_FILE="$OUTPUT_DIR/cursor_config_ubuntu.json"
...
if command -v cursor &> /dev/null; then
    VERSION=$(cursor --version 2>/dev/null || echo "unknown")
    echo "    \"installed\": true," >> "$REPORT_FILE"
    echo "    \"version\": \"$VERSION\"," >> "$REPORT_FILE"
    echo "    \"path\": \"$(which cursor)\"," >> "$REPORT_FILE"
else
...
if [ -d "$EXTENSIONS_DIR" ]; then
    EXT_COUNT=$(find "$EXTENSIONS_DIR" -maxdepth 2 -name "package.json" 2>/dev/null | wc -l | tr -d ' ')
