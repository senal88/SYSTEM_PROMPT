...
main() {
    ...
    if [ -f "$REPO_ROOT/configs/extensions/recommended.json" ]; then
        log "Aplicando perfil de extensões..."
        cp "$REPO_ROOT/configs/extensions/recommended.json" "$CURSOR_CONFIG_DIR/extensions/recommended.json"
        success "Perfil de extensões aplicado"
    else
        warning "Arquivo extensions/recommended.json não encontrado"
    fi
