...
# ============================================
# Script de Coleta: Ferramentas de IA - macOS
# ============================================
# Detecta ferramentas de IA instaladas no sistema
# ============================================
...
REPORT_FILE="$OUTPUT_DIR/ia_tools_macos.json"
...
if command -v python3 &> /dev/null; then
    ...
    for pkg in openai anthropic google-generativeai transformers; do
        if python3 -c "import ${pkg//-/_}" 2>/dev/null; then
            VERSION=$(python3 -c "import ${pkg//-/_}; print(${pkg//-/_}.__version__)" 2>/dev/null || echo "installed")
...
APPS=("Cursor.app" "ChatGPT.app" "Perplexity.app" "Claude.app")
for app in "${APPS[@]}"; do
    APP_NAME="${app%.app}"
    if [ -d "/Applications/$app" ]; then
        VERSION=$(defaults read "/Applications/$app/Contents/Info.plist" CFBundleShortVersionString 2>/dev/null || echo "unknown")
        echo "      \"$APP_NAME\": {" >> "$REPORT_FILE"
        echo "        \"installed\": true," >> "$REPORT_FILE"
        echo "        \"version\": \"$VERSION\"," >> "$REPORT_FILE"
        echo "        \"path\": \"/Applications/$app\"" >> "$REPORT_FILE"
        echo "      }," >> "$REPORT_FILE"
    else
        echo "      \"$APP_NAME\": {" >> "$REPORT_FILE"
        echo "        \"installed\": false" >> "$REPORT_FILE"
