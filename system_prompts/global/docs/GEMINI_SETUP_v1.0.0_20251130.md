...
2.  **Exportação via Variável de Ambiente:**
    ...
    - O método preferencial é usar o `op` para carregar a variável dinamicamente:
      ```bash
      export GOOGLE_API_KEY=$(op read "op://<vault>/Gemini API Key/credential")
      ```
    - O script `scripts/auditar-1password-secrets...sh` automatiza essa verificação e exportação.
