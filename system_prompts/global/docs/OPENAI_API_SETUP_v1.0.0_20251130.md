...
2.  **Exportação como Variável de Ambiente:**
    ...
    - O `~/.zshrc` (macOS) ou `~/.bashrc` (VPS) deve ser configurado para carregar esta variável usando o 1Password CLI, garantindo que a chave nunca seja escrita em texto plano no disco.
      ...
      export OPENAI_API_KEY=$(op read "op://<vault>/OpenAI API Key/credential")
      ```
    - A execução do script `scripts/auditar-1password-secrets...sh` deve validar se esta variável está sendo carregada corretamente no seu ambiente de shell.
