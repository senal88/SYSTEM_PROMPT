#!/bin/bash
# Script para consolidar documentação para upload no Claude Cloud Pro
...
OUTPUT_DIR="$DOTFILES_HOME/claude-cloud-knowledge"
...
# 1.1 Ambientes
if [ -f "$DOTFILES_HOME/context-engineering/CONTEXTO_AMBIENTES_COMPLETO.md" ]; then
    cp "$DOTFILES_HOME/context-engineering/CONTEXTO_AMBIENTES_COMPLETO.md" \
...
# 2. Configurações
log_info "Consolidando Configurações..."

# 2.1 1Password
if [ -f "$DOTFILES_HOME/automation_1password/README.md" ]; then
    cp "$DOTFILES_HOME/automation_1password/README.md" \
       "$OUTPUT_DIR/01_CONFIGURACOES/1Password.md"
    log_success "1Password.md"
fi

# 2.2 GitHub
if [ -f "$DOTFILES_HOME/automation_1password/scripts/UPDATE_DATASETS.md" ]; then
    # Extrair apenas seção GitHub
    sed -n '/^## GitHub/,/^## Hugging Face/p' \
        "$DOTFILES_HOME/automation_1password/scripts/UPDATE_DATASETS.md" > \
...
# 2.3 Hugging Face
if [ -f "$DOTFILES_HOME/automation_1password/scripts/UPDATE_DATASETS.md" ]; then
    # Extrair apenas seção Hugging Face
...
# 3. Projeto BNI
log_info "Consolidando Documentação do Projeto..."
...
# 4. Automação
log_info "Consolidando Automação..."
...
## Localização
Todos os scripts em: `$DOTFILES_HOME/automation_1password/scripts/`
EOF
log_success "Scripts.md"

# 4.2 Integrações
cat > "$OUTPUT_DIR/03_AUTOMACAO/Integrações.md" << 'EOF'
...
EOF
log_success "Integrações.md"

# 4.3 Deploy
if [ -f "$DOTFILES_HOME/context-engineering/PLANO_ACAO_FINAL.md" ]; then
    # Extrair seção de deploy
    sed -n '/^## FASE 3: Automação e Deploy/,/^## FASE 4:/p' \
        "$DOTFILES_HOME/context-engineering/PLANO_ACAO_FINAL.md" > \
        "$OUTPUT_DIR/03_AUTOMACAO/Deploy.md" || true
    log_success "Deploy.md"
fi

# 5. Referências
log_info "Consolidando Referências..."

# 5.1 Guias
if [ -f "$DOTFILES_HOME/context-engineering/GUIA_RAPIDO.md" ]; then
    cp "$DOTFILES_HOME/context-engineering/GUIA_RAPIDO.md" \
...
log_info "Criando índice principal..."
...
- **Infraestrutura.md** - Stack tecnológica e serviços
- **Stack.md** - Tecnologias e ferramentas utilizadas

### 01_CONFIGURACOES/
- **1Password.md** - Sistema completo de automação 1Password
- **GitHub.md** - Configuração e integração GitHub
- **HuggingFace.md** - Configuração e integração Hugging Face

### 02_PROJETO_BNI/
- **Contexto.md** - Contexto completo do projeto BNI
- **Skills.md** - Skills e especializações
- **Credenciais.md** - Política de credenciais e segurança

### 03_AUTOMACAO/
- **Scripts.md** - Scripts disponíveis e uso
- **Integrações.md** - Integrações configuradas
- **Deploy.md** - Processo de deploy e automação

### 04_REFERENCIAS/
- **Guias.md** - Guias rápidos de uso
- **API.md** - Referências de APIs
- **Troubleshooting.md** - Solução de problemas comuns

## 🚀 Como Usar

1. Faça upload de todos os arquivos .md no Claude Cloud Pro
2. Organize em pastas conforme estrutura acima
3. Use como referência nas conversas com Claude

## 📝 Atualização

Execute `consolidate-docs-for-claude.sh` para atualizar a documentação.
...
# Criar arquivo consolidado único (opcional)
log_info "Criando arquivo consolidado único..."
