...
# Script de Injeção Dinâmica de Segredos do 1Password para VPS Ubuntu
...
check_prerequisites() {
    ...
    if ! command -v op &> /dev/null; then
        ...
        log_info "Instale com: sudo apt-get install 1password-cli"
        exit 1
    fi

    # Verificar se OP_SERVICE_ACCOUNT_TOKEN está definido
    if [[ -z "${OP_SERVICE_ACCOUNT_TOKEN:-}" ]]; then
        log_error "OP_SERVICE_ACCOUNT_TOKEN não está definido."
        log_info "Configure com: export OP_SERVICE_ACCOUNT_TOKEN='seu_token_aqui'"
        exit 1
    fi
...
display_summary() {
    ...
    log_info "Permissões: $(stat -c '%A' "$ENV_OUTPUT_FILE" 2>/dev/null || echo 'N/A')"
...
main() {
    echo ""
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║  🔓 Injeção de Segredos do 1Password para VPS Ubuntu           ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
