...
log_info "🚀 Configurando GitHub CLI e Git..."
...
if ! op item get "GitHub Token" --vault "$VAULT" &>/dev/null; then
    log_warning "Token do GitHub não encontrado no vault $VAULT"
...
if command -v gh &>/dev/null; then
    ...
    if op item get "GitHub Token" --vault "$VAULT" &>/dev/null; then
        ...
        log_warning "Token não encontrado. Execute login manual: gh auth login"
    fi
else
    log_warning "GitHub CLI (gh) não instalado. Instale via Homebrew: brew install gh"
fi
...
if ! git config --global user.name &>/dev/null; then
    ...
    log_info "Git user.name já configurado: $(git config --global user.name)"
fi
...
if ! git config --global user.email &>/dev/null; then
    ...
    log_info "Git user.email já configurado: $(git config --global user.email)"
fi

git config --global init.defaultBranch main
log_success "Git branch padrão configurado: main"
...
if op item get "GitHub SSH Key" --vault "$VAULT" &>/dev/null; then
    ...
    # Obter chave pública (se existir)
...
EOF
            ...
            log_info "SSH config já contém GitHub"
        fi
    fi
else
    log_warning "Chave SSH não encontrada no 1Password"
    log_info "Para adicionar: op item create --category=ssh_key --title='GitHub SSH Key' --vault=$VAULT"
fi

# 5. Configurar dotfiles no GitHub (se aplicável)
log_info "Verificando repositório dotfiles..."

DOTFILES_DIR="$DOTFILES_HOME"
if [ -d "$DOTFILES_DIR/.git" ]; then
    log_info "Repositório Git encontrado em $DOTFILES_DIR"

    # Verificar remote
    if ! git -C "$DOTFILES_DIR" remote get-url origin &>/dev/null; then
        read -p "URL do repositório GitHub para dotfiles (ou Enter para pular): " REPO_URL
        ...
        log_info "Remote origin já configurado: $(git -C "$DOTFILES_DIR" remote get-url origin)"
    fi
else
    log_info "Diretório dotfiles não é um repositório Git"
fi

# 6. Configurar Codespaces (se no macOS)
if [[ "$OSTYPE" == "darwin"* ]]; then
    log_info "Configurações para Codespaces disponíveis via .devcontainer/"
fi

log_success "✅ Setup do GitHub concluído!"
log_info ""
